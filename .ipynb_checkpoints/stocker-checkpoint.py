...
class Stocker():
  ...
  # Read csv file
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Close'], plot_type='basic'):
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        start_price = float(self.stock[self.stock['Date'] == start_date]['Open'])
        end_price = float(self.stock[self.stock['Date'] == end_date]['Close'])
        
        # Make a profit dataframe and calculate profit column
        profits = self.make_df(start_date, end_date)
        profits['hold_profit'] = nshares * (profits['Close'] - start_price)
