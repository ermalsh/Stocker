...
class Stocker():
    ...
    def __init__(self, ticker, exchange='WIKI'):
        ...
        quandl.ApiConfig.api_key = '4ZJCXfPsxZJSRgLqpzpy'



  # Read csv file
        stock = pd.read_csv(os.path.join("data", "%s.csv" % ticker))
    ...
    def handle_dates(self, start_date, end_date):
        ...
        while (not valid_start) & (not valid_end):
            ...
            if end_date < start_date:
                ...
                if end_date > pd.to_datetime(self.max_date):
                    print('End Date exceeds data range')
                    end_date= pd.to_datetime(input('Enter a new end date: '))
                    valid_end = False

                if start_date < pd.to_datetime(self.min_date):
    ...
    def make_df(self, start_date, end_date, df=None):
        ...
        if self.round_dates:
            ...
            if (not end_in) & (not start_in):
                trim_df = df[(pd.to_datetime(df['Date']) >= start_date) & 
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock.ix[len(self.stock) - 1, stat], self.max_date))
            ...
            if plot_type == 'pct':
                ...
                if stat == 'Daily Change':
                    plt.plot(pd.to_datetime(stock_plot['Date']), 100 * stock_plot[stat],
                         color = colors[i], linewidth = 2.4, alpha = 0.9,
                         label = stat)
                else:
                    plt.plot(pd.to_datetime(stock_plot['Date']), 100 * (stock_plot[stat] -  stat_avg) / stat_avg,
                         color = colors[i], linewidth = 2.4, alpha = 0.9,
                         label = stat)
            ...
            # Stat y-axis
                ...
                plt.style.use('fivethirtyeight');
                plt.plot(pd.to_datetime(stock_plot['Date']), stock_plot[stat], color = colors[i], linewidth = 3, label = stat, alpha = 0.8)
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        start_price = float(self.stock[pd.to_datetime(pd.to_datetime(self.stock['Date'])) == start_date]['Open'])
        end_price = float(self.stock[pd.to_datetime(pd.to_datetime(self.stock['Date'])) == end_date]['Adj Close'])
        ...
        plt.plot(pd.to_datetime(profits['Date']), profits['hold_profit'], 'b', linewidth = 3)
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        train = self.stock[(pd.to_datetime(self.stock['Date']) > (max(pd.to_datetime(self.stock['Date'])) - pd.DateOffset(years=self.training_years)))]
        ...
        # Actual observations
        ax.plot(pd.to_datetime(train['ds']), train['y'], 'ko', ms = 4, label = 'Observations')
        ...
        for prior in changepoint_priors:
            ...
            ax.fill_between(predictions['ds'], predictions['%.3f_yhat_upper' % prior],
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        stock_history = self.stock[pd.to_datetime(pd.to_datetime(self.stock['Date'])) > (pd.to_datetime(self.max_date) - pd.DateOffset(years = self.training_years))]
        ...
        ax.plot(pd.to_datetime(stock_history['ds']), stock_history['y'], 'ko-', linewidth = 1.4, alpha = 0.8, ms = 1.8, label = 'Observations')
        
        # Plot the predicted values
        ax.plot(pd.to_datetime(future['ds']), future['yhat'], 'forestgreen',linewidth = 2.4, label = 'Modeled');

        # Plot the uncertainty interval as ribbon
        ax.fill_between(pd.to_datetime(future['ds']).dt.to_pydatetime(), future['yhat_upper'], future['yhat_lower'], alpha = 0.3, 
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if start_date is None:
            start_date = pd.to_datetime(self.max_date) - pd.DateOffset(years=1)
        if end_date is None:
            end_date = pd.to_datetime(self.max_date)
            
        start_date, end_date = self.handle_dates(start_date, end_date)
        
        # Training data starts self.training_years years before start date and goes up to start date
        train = self.stock[(pd.to_datetime(pd.to_datetime(self.stock['Date'])) < start_date) & 
                           (pd.to_datetime(self.stock['Date']) > (start_date - pd.DateOffset(years=self.training_years)))]
        
        # Testing data is specified in the range
        test = self.stock[(pd.to_datetime(self.stock['Date']) >= start_date) & (pd.to_datetime(self.stock['Date']) <= end_date)]
        ...
        train = pd.merge(test, future, on="ds", how = 'inner')
        ...
        train_errors = abs(train['y'] - train['yhat_y'])
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        train = self.stock[pd.to_datetime(pd.to_datetime(self.stock['Date'])) > (pd.to_datetime(self.max_date) - pd.DateOffset(years = self.training_years))]
        model.fit(train)
        ...
        if not search:
            ...
            # Set up line plot 
            plt.plot(pd.to_datetime(train['ds']), train['y'], 'ko', ms = 4, label = 'Stock Price')
            plt.plot(pd.to_datetime(future['ds']), future['yhat'], color = 'navy', linewidth = 2.0, label = 'Modeled')
            
            # Changepoints as vertical lines
            plt.vlines(pd.to_datetime(cpos_data['ds']).dt.to_pydatetime(), ymin = min(train['y']), ymax = max(train['y']), 
                       linestyles='dashed', color = 'r', 
                       linewidth= 1.2, label='Negative Changepoints')

            plt.vlines(pd.to_datetime(cneg_data['ds']).dt.to_pydatetime(), ymin = min(train['y']), ymax = max(train['y']), 
    ...
    def predict_future(self, days=30):
        ...
        future = future[pd.to_datetime(future['ds']) >= max(pd.to_datetime(self.stock['Date']))]
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None,changepoint_priors = [0.001, 0.05, 0.1, 0.2]):
        ...
        test = self.stock[(pd.to_datetime(self.stock['Date']) >= pd.to_datetime(start_date)) & (pd.to_datetime(self.stock['Date']) <= pd.to_datetime(end_date))]
        eval_days = (max(pd.to_datetime(test['Date'])) - min(pd.to_datetime(test['Date']))).days
